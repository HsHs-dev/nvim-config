...
return {
	...
	-- ── nvim-lspconfig + core LSP setup ───────────────────────
	{
		...
		config = function()
			...
			-- Force undercurl in terminals that support it
			vim.cmd([[
        let &t_Cs = "\e[4:3m"
        let &t_Ce = "\e[4:0m"
      ]])
			-- Diagnostic colors matching VSCode's default theme
			vim.cmd([[
        hi DiagnosticUnderlineError guisp=#f14c4c gui=undercurl
        hi DiagnosticUnderlineWarn  guisp=#cca700 gui=undercurl
        hi DiagnosticUnderlineInfo  guisp=#3794ff gui=undercurl
        hi DiagnosticUnderlineHint  guisp=#3794ff gui=undercurl
      ]])
